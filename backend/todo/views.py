...
class ToDoCategoryView(APIView):
    ...
    def delete(self, request):
        # delete category
        try:
            category = ToDoCategory.objects.get(author=request.user, id=request.data['cat_id'])
            category.delete()
...
class ToDoItemView(APIView):
    ...
    def post(self, request):
        ...
        if not ToDoCategory.objects.filter(author=request.user ,id=category_id).exists():
        ...
        todo_item = ToDoItem.objects.create(category_id=category_id, item=request.data['item'], isDone=isDone)
        todo_item.save()
    ...
    def put(self, request):
        ...
        if not ToDoItem.objects.filter(category__author=request.user, id=todo_item).exists():
